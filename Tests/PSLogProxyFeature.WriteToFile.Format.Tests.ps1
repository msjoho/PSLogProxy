BeforeAll {
    ...
    Set-PSLogProxySetting -Name DateTimeStringScriptblock -Value { 'TEST_DT' }
...
AfterAll {
    Set-PSLogProxySetting -Name DateTimeStringScriptblock -Value { Get-Date -Format u }
    ...
    Set-PSLogProxyFeatureSetting -FeatureName 'LogFile' -Name 'AddPrefixStream' -Value $true
    Set-PSLogProxyFeatureSetting -FeatureName 'LogFile' -Name 'AddPrefixInvocationScript' -Value $true
    Set-PSLogProxyFeatureSetting -FeatureName 'LogFile' -Name 'ExcludeMessageRegex' -Value @()
}
...
Describe -tag 'Skip' 'WriteToFile Feature Log Entry Format' {
    ...
    Context 'DateTime prefix only' {
        BeforeEach {
            Set-PSLogProxyFeatureSetting -FeatureName 'LogFile' -Name 'AddPrefixDateTimeString' -Value $true
            Set-PSLogProxyFeatureSetting -FeatureName 'LogFile' -Name 'AddPrefixStream' -Value $false
            Set-PSLogProxyFeatureSetting -FeatureName 'LogFile' -Name 'AddPrefixInvocationScript' -Value $false
        }
    ...
    Context 'ExcludeMessageRegex' {
        BeforeEach {
            ...
            Set-PSLogProxyFeatureSetting -FeatureName 'LogFile' -Name 'AddPrefixStream' -Value $false
            Set-PSLogProxyFeatureSetting -FeatureName 'LogFile' -Name 'AddPrefixInvocationScript' -Value $false
            Set-PSLogProxyFeatureSetting -FeatureName 'LogFile' -Name 'ExcludeMessageRegex' -Value @('EXCLUDEME')
        }
        ...
        It 'multiple regex patterns: any match excludes the message' {
            Set-PSLogProxyFeatureSetting -FeatureName 'LogFile' -Name 'ExcludeMessageRegex' -Value @('PATTERN_A', 'PATTERN_B')
            Write-Verbose 'AnchorMessage' -Verbose 4>&1
